...
for (const mode of ["copy", "transcode"] as const)
  test(
    `real upload, normalization, preview and local RTMP broadcast (${mode})`,
    { timeout: 60000 },
    async (t) => {
      try {
      ...
      try {
        ...
        await until(
          () => f.store.get("videos", accepted.id)?.status === "ready",
        );
        ...
        f.store.save("streams", {
          ...stream,
          overlayText:
            mode === "transcode" ? "StreaMax: 100% 'live' [test]" : "",
          watermark: mode === "transcode",
          livePreview: true,
        });
        f.store.save("playlists", { ...playlist, videoIds: [video.id] });
        f.store.save("profiles", {
          ...profile,
          width: mode === "copy" ? 1280 : 640,
          height: mode === "copy" ? 720 : 360,
          bitrate: 600,
        });
        ...
        assert.equal(
          (await f.request("POST", `streams/${stream.id}/start`, {}))
            .statusCode,
          200,
        );
        await until(
          () => f.engine.status(stream.id)[0]?.state === "live",
          15000,
        );
        assert.equal(f.engine.status(stream.id)[0].mode, mode);
        ...
        assert.ok(segment);
        assert.equal(
          (await f.request("GET", `preview/${stream.id}/${segment}`))
            .statusCode,
          200,
        );
        assert.equal(
